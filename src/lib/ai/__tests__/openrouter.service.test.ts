...
describe("OpenRouterService", () => {
  ...
  describe("Constructor", () => {
    ...
    it("should throw error when API key is missing", () => {
      expect(() => new OpenRouterService({ ...config, apiKey: "" })).toThrow(
    ...
    it("should throw error when app referer is missing", () => {
      expect(
        () => new OpenRouterService({ ...config, appReferer: "" }),
      ).toThrow("App referer is required");
    });

    it("should throw error when app title is missing", () => {
      expect(() => new OpenRouterService({ ...config, appTitle: "" })).toThrow(
  ...
  describe("chatJson", () => {
    ...
    it("should parse JSON from content with extra text", async () => {
      const content =
        'Here is the JSON: ["item1", "item2"] and some extra text';
      mockFetch.mockResolvedValueOnce({
        ok: true,
        json: () =>
          Promise.resolve({
    ...
    it("should throw error when JSON parsing fails", async () => {
      ...
      await expect(
        service.chatJson(mockMessages, mockJsonSchema),
      ).rejects.toThrow("Failed to locate JSON in content");
    });
  });

  describe("Error handling", () => {
    const mockMessages: ChatMessage[] = [{ role: "user", content: "test" }];
