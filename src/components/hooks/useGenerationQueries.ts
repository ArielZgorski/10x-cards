...
function createMockGeneration(id: UUID): AIGenerationDTO {
  const mockGenerations: Record<string, Partial<AIGenerationDTO>> = {
    'test-generation-running': {
      status: 'running',
      source_text: 'Tekst źródłowy do generowania fiszek o programowaniu React...',
    },
  ...
  const baseGeneration: AIGenerationDTO = {
    id,
    user_id: 'user-123',
    source_text: 'Przykładowy tekst źródłowy do generowania fiszek z pojęciami programistycznymi, które powinny być przekształcone w karty do nauki.',
    model: 'gpt-4',
    prompt_version: 'v1.2',
    tokens_input: 250,
    tokens_output: 800,
...
function createMockSuggestions(generationId: UUID): AISuggestionDTO[] {
  const baseSuggestions: Omit<AISuggestionDTO, 'id' | 'front' | 'back' | 'status'>[] = [
    {
      user_id: 'user-123',
  ...
  const suggestionData = [
    { front: 'Czym jest React Hook?', back: 'React Hook to funkcja, która pozwala na używanie stanu i innych funkcjonalności React w komponentach funkcyjnych.', status: 'proposed' as const },
    { front: 'Co to jest useState?', back: 'useState to React Hook, który pozwala na dodawanie lokalnego stanu do komponentów funkcyjnych.', status: 'proposed' as const },
    { front: 'Jak działa useEffect?', back: 'useEffect to Hook, który pozwala na wykonywanie efektów ubocznych w komponentach funkcyjnych, takich jak pobieranie danych czy subskrypcje.', status: 'edited' as const },
    { front: 'Co oznacza JSX?', back: 'JSX (JavaScript XML) to rozszerzenie składni JavaScript, które pozwala na pisanie struktur HTML-podobnych w kodzie JavaScript.', status: 'accepted' as const },
    { front: 'Czym jest komponent w React?', back: 'Komponent to niezależna, wielokrotnego użytku część kodu, która zwraca elementy React opisujące, co powinno pojawić się na ekranie.', status: 'rejected' as const },
    { front: 'Co to jest props w React?', back: 'Props (properties) to argumenty przekazywane do komponentów React. Działają podobnie jak argumenty funkcji.', status: 'proposed' as const },
  ];
  ...
  return suggestionData.map((data, index) => ({
    ...
    ...baseSuggestions[0],
    ...data,
    accepted_at: data.status === 'accepted' ? new Date(Date.now() - 1800000).toISOString() : null, // 30 min ago if accepted
  ...
  return useQuery({
    ...
    refetchInterval: (data, query) => {
      ...
      if (status === 'pending' || status === 'running') {
        ...
        const interval = Math.min(baseInterval * Math.pow(1.5, failureCount), maxInterval);
        return interval;
      }
