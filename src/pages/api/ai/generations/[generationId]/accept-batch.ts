...
export const POST: APIRoute = async ({ request, locals, params }) => {
  ...
  try {
    ...
    const created: Array<{ suggestion_id: string; card_id: string }> = [];
    const failed: Array<{ suggestion_id: string; error: string }> = [];
    ...
    const notFoundIds = suggestion_ids.filter(id => !foundSuggestionIds.includes(id));
